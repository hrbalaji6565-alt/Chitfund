...
async function getLoanData(loanId: string) {
  try {
    ...
    const memberId = (loan as any).userId;
    if (!memberId) {
      return null;
    }
    const memberIdStr = String(memberId);

    const member = mongoose.Types.ObjectId.isValid(memberIdStr)
      ? await Member.findById(memberIdStr).select("_id name userId mobile").lean()
      : null;

    const allPayments = await Payment.find({
      memberId: mongoose.Types.ObjectId.isValid(memberIdStr)
    ...
    const schedule = Array.isArray((loan as any).schedule) ? (loan as any).schedule : [];
    const now = new Date();
    ...
    return {
      ...
      memberId: serializeValue(memberRec._id || (loan as any).userId),
      memberName: String((loan as any).memberName || memberRec.name || "Unknown"),
      memberUserId: String(memberRec.userId || ""),
      memberMobile: String(memberRec.mobile || ""),
      ...
      durationType: String((loan as any).durationType || "MONTHS"),
      durationValue: Number((loan as any).durationValue || (loan as any).durationMonths || 0),
      startDate: serializeValue((loan as any).startDate),
      endDate: serializeValue((loan as any).endDate),
    ...
    console.error("Loan profile page data load failed:", error);
    return null;
  }
}
